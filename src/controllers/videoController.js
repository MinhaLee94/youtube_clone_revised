...
export const watch = async (req, res) => {
  ...
  const moreVideos = await Video.find({ _id: { $ne: id }}).sort({createdAt: "desc"}).populate("owner");
  if(!video){
    return res.status(404).render("404", { pageTitle: "Video not found." });
  }
  return res.render("watch", { pageTitle: video.title, video, moreVideos });
};
...
export const postUpload = async (req, res) => {
  ...
  try {
    const newVideo = await Video.create ({
      ...
      thumbUrl: thumb[0].location, //thumb[0].destination+thumb[0].filename,
